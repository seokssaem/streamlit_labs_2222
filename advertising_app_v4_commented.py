...
# ─────────────────────────────────────────────────────────────
# 광고 데이터셋 선형회귀 Streamlit 대시보드
# - 데이터: Kaggle Advertising Dataset(TV, Radio, Newspaper, Sales)
# - 목표: 광고비를 바탕으로 Sales를 예측하는 단순/다중 선형회귀 실습
# - 주의: 아래 회귀 모델은 StandardScaler로 입력값을 표준화한 뒤 학습합니다.
#         따라서 화면에 표시되는 계수 w는 '원본 광고비 1달러 증가 효과'가 아니라
#         '표준화된 입력값 1단위 증가 효과'입니다.
# ─────────────────────────────────────────────────────────────
...
def load_data():
    ...
    try:
        ...
        # 파일을 못 찾았을 때 앱이 멈추지 않도록 예비 샘플 데이터를 생성함
        # 실제 수업/평가에서는 input/advertising.csv가 잘 들어있는지 확인하는 것이 좋음
        np.random.seed(42)
        n = 200
...
with tab1:
    ...
    st.dataframe(df.sample(5, random_state=42), width='stretch')
...
with tab3:
    ...
    # 사용자가 테스트 데이터 비율과 랜덤 시드를 바꿔보며
    # 모델 성능이 어떻게 달라지는지 확인할 수 있게 함
    ...
    # 전처리 & 학습
    # X1: 독립변수. 단순 선형 회귀이므로 TV 광고비 1개만 사용
    # y1: 종속변수. 예측하고 싶은 값인 Sales 사용
    X1 = df[["TV"]]
    y1 = df["Sales"]

    # StandardScaler는 평균 0, 표준편차 1 형태로 값을 변환함
    # 선형회귀 자체에는 필수는 아니지만, 계수 크기를 비교하거나
    # 스케일 차이를 줄이는 실습용으로 사용함
    ...
    m3.metric("기울기 (w, 표준화 기준)", f"{w1:.2f}")
    m4.metric("절편 (b)", f"{b1:.2f}")

    st.info(f"📌 회귀식(표준화 기준): **y = {w1:.2f}x_scaled + {b1:.2f}**")
    ...
    # ③ 회귀선
    # 선 그래프는 x값 순서대로 정렬해야 회귀선이 지그재그로 보이지 않음
    sort_idx1 = np.argsort(X_te1.ravel())
    axes4[2].scatter(X_scaled1, y1, color="red", s=15, alpha=0.5, label="전체 데이터")
    axes4[2].plot(X_te1[sort_idx1], y_pred1[sort_idx1], color="blue", linewidth=2, label="회귀선")
    ...
    tv_scaled = scaler1.transform(pd.DataFrame([[tv_input]], columns=["TV"]))
...
with tab4:
    ...
    # X2: 다중 선형 회귀의 독립변수 3개(TV, Radio, Newspaper)
    # y2: 예측 대상인 Sales
    X2 = df.drop(columns=["Sales"])
    y2 = df["Sales"]

    # 광고 플랫폼별 단위 범위가 다르므로 표준화 후 학습
    # 이때 계수는 '표준화 기준 영향력'으로 해석하는 것이 적절함
    ...
    st.info(f"📌 회귀식(표준화 기준): **{formula.replace('TV', 'TV_scaled').replace('Radio', 'Radio_scaled').replace('Newspaper', 'Newspaper_scaled')}**")
    ...
    ax6.set_xlabel("표준화 계수(standardized coefficient)")
    ax6.set_title("광고 플랫폼별 판매 영향력(표준화 기준)")
    ...
    inp_scaled = scaler2.transform(pd.DataFrame([[tv_in, radio_in, news_in]], columns=["TV", "Radio", "Newspaper"]))
